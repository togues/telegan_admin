<?php
/**
 * Módulo: Usuarios Duplicados/Sospechosos - Listado
 */
...
<head>
    ...
    <title>Usuarios Duplicados - Telegan Admin</title>
...
<body>
    ...
    <main class="main-content">
        <div class="content-header">
            <h1 class="page-title">Usuarios Duplicados/Sospechosos</h1>
            <p class="page-subtitle">Usuarios que parecen ser el mismo (emails, teléfonos o códigos Telegan duplicados)</p>
        </div>
            ...
            <table class="users">
                <thead class="sticky">
                    <tr>
                        ...
                        <th style="width:100px;">Duplicados</th>
                    </tr>
                </thead>
                <tbody id="tbody">
    ...
    <script type="module">
        ...
        function buildUrl(includePagination = true) {
            ...
            return `api/alerts-duplicate-users.php?${params.toString()}`;
        }
        ...
        async function loadUsers() {
            try {
                ...
                els.tbody.innerHTML = `<tr><td colspan="10" style="text-align:center; color: var(--error-color); padding: 16px;">Error: ${e.message || 'Error desconocido'}</td></tr>`;
            }
        }
        ...
        function renderRows(rows) {
            ...
            const motivoLabels = {
                'email_duplicado': 'Email Duplicado',
                'telefono_duplicado': 'Teléfono Duplicado',
                'codigo_telegan_duplicado': 'Código Telegan Duplicado',
                'duplicado': 'Duplicado'
            };
            const html = rows.map(r => `
                <tr>
                    ...
                    <td><span style="padding: 0.25rem 0.5rem; border-radius: 4px; background: var(--bg-secondary); font-size: 0.85rem;">${motivoLabels[r.motivo] || r.motivo || 'Duplicado'}</span></td>
                    <td style="text-align: center;">${r.duplicados_count || 1}</td>
                </tr>
            `).join('');
        ...
        async function downloadUsers() {
            ...
            try {
                ...
                const exportData = users.map(user => ({
                    ...
                    'Motivo': user.motivo || 'Duplicado',
                    'Duplicados': user.duplicados_count || 1
                }));

                const ws = XLSX.utils.json_to_sheet(exportData);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, 'Usuarios Duplicados');
                ...
                const fileName = `usuarios_duplicados_${dateStr}.xlsx`;
