<?php
/**
 * API Dashboard - Con validación gradual de seguridad
 * 
 * Ejemplo de implementación del middleware de seguridad
 * que NO rompe funcionalidad durante desarrollo
 */

// Incluir middleware de seguridad y autenticación API
require_once '../../src/Middleware/SecurityMiddleware.php';
require_once '../../src/Config/ApiAuth.php';

// Inicializar middleware
SecurityMiddleware::init();

// Configurar headers de respuesta
header('Content-Type: application/json');
header('Access-Control-Allow-Origin: *'); // Mantener abierto durante desarrollo
header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
header('Access-Control-Allow-Headers: Content-Type, Authorization, X-API-Token, X-API-Timestamp, X-Session-Token');

// Manejar preflight requests
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    http_response_code(200);
    exit();
}

// Función para enviar respuesta JSON
function sendResponse($data, $status = 200) {
    http_response_code($status);
    echo json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
    exit();
}

// Función para manejar errores
function handleError($message, $status = 500) {
    sendResponse([
        'success' => false,
        'error' => $message,
        'timestamp' => date('Y-m-d H:i:s')
    ], $status);
}

try {
    // Verificar método HTTP
    if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
        handleError('Método no permitido', 405);
    }
    
    // Validar token de autenticación (ApiAuth normaliza el path automáticamente)
    $validation = ApiAuth::validateRequest();
    
    if (!$validation['valid']) {
        handleError('Acceso no autorizado: ' . ($validation['error'] ?? 'Token inválido'), 401);
    }

    // VALIDACIÓN GRADUAL DE SEGURIDAD
    // Opción 1: Solo logging (desarrollo)
    SecurityMiddleware::publicApi();
    
    // Opción 2: Requerir token de aplicación (cuando esté listo)
    // SecurityMiddleware::requireAppToken(false); // false = no estricto en desarrollo
    
    // Opción 3: Requerir autenticación completa (producción)
    // SecurityMiddleware::requireAuth(true); // true = estricto
    
    // Incluir dependencias
    require_once '../../src/Config/Database.php';
    require_once '../../src/Models/Dashboard.php';

    // Crear instancia del modelo Dashboard
    $dashboard = new Dashboard();

    // Verificar conexión a la base de datos
    if (!$dashboard->verificarConexion()) {
        handleError('Error de conexión a la base de datos', 503);
    }

    // Obtener datos del dashboard
    $estadisticas = [
        'total_usuarios' => $dashboard->getTotalUsuarios(),
        'total_fincas' => $dashboard->getTotalFincas(),
        'total_potreros' => $dashboard->getTotalPotreros(),
        'total_registros_ganaderos' => $dashboard->getTotalRegistrosGanaderos(),
        'usuarios_activos' => $dashboard->getUsuariosActivos(),
        'usuarios_administradores' => $dashboard->getUsuariosAdministradores(),
        'usuarios_colaboradores' => $dashboard->getUsuariosColaboradores(),
    ];

    // Obtener alertas
    $alertas = [
        'usuarios_sin_finca' => $dashboard->getUsuariosSinFinca()['total_usuarios_sin_finca'] ?? 0,
        'fincas_sin_potreros' => $dashboard->getFincasSinPotreros()['fincas_sin_potreros'] ?? 0,
        'usuarios_inactivos' => $dashboard->getUsuariosInactivos()['usuarios_inactivos_30dias'] ?? 0,
        'fincas_sin_actividad' => $dashboard->getFincasSinActividad()['fincas_sin_actividad_30dias'] ?? 0,
    ];

    // Información del sistema
    $sistema = [
        'version' => '1.0.0',
        'entorno' => 'development', // Simplificado para evitar problemas con $_ENV
        'timestamp' => date('Y-m-d H:i:s'),
        'servidor' => $_SERVER['HTTP_HOST'] ?? 'localhost',
        'base_datos' => $dashboard->getInfoBaseDatos(),
        'seguridad' => [
            'app_token_validado' => !empty($_SERVER['HTTP_X_APP_TOKEN']),
            'auth_header_presente' => !empty($_SERVER['HTTP_AUTHORIZATION']),
            'modo_desarrollo' => true // Simplificado
        ]
    ];

    sendResponse([
        'success' => true,
        'data' => [
            'estadisticas' => $estadisticas,
            'alertas' => $alertas,
            'sistema' => $sistema
        ],
        'timestamp' => date('Y-m-d H:i:s')
    ]);

} catch (Exception $e) {
    error_log("Error en dashboard.php: " . $e->getMessage());
    handleError('Error interno del servidor', 500);
}
?>

